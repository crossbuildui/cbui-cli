...
const API_BASE_URL = 'https://www.crossbuildui.com/';
const SECURE_PACKAGES_ENDPOINT = `${API_BASE_URL}/api/packages`;

/**
 * Normalizes package names to ensure they have the @crossbuildui/ prefix
 * @param {string[]} packageNames - Array of package names (with or without @crossbuildui/ prefix)
 * @returns {string[]} - Array of normalized package names with @crossbuildui/ prefix
 */
function normalizePackageNames(packageNames) {
    return packageNames.map(packageName => {
        // If the package name already starts with @crossbuildui/, return as is
        if (packageName.startsWith('@crossbuildui/')) {
            return packageName;
        }

        // If it starts with @, it might be a different scoped package, return as is
        if (packageName.startsWith('@')) {
            return packageName;
        }

        // Otherwise, add the @crossbuildui/ prefix
        return `@crossbuildui/${packageName}`;
    });
}

/**
 * Downloads and extracts/saves packages from the secure endpoint.
 * @param {string[]} packageNames - Array of package filenames (e.g., ["component-a.tgz", "component-b.tgz"]) or component names (e.g., ["biometricunlock", "footernavbar"]).
 * @param {string} idToken - Firebase Auth ID token.
 */
export async function downloadPackage(packageNames, idToken) {
    // Normalize package names to ensure they have the @crossbuildui/ prefix
    const normalizedPackageNames = normalizePackageNames(packageNames);

    const mainSpinner = ora(`Preparing to download components: ${chalk.bold(normalizedPackageNames.join(', '))}`).start();

    try {
        await fs.ensureDir(DOWNLOADS_DIR);

        mainSpinner.text = `Requesting ${normalizedPackageNames.length} component(s) from server...`;
        const response = await fetch(SECURE_PACKAGES_ENDPOINT, {
            ...
            body: JSON.stringify({ components: normalizedPackageNames }),
        });
